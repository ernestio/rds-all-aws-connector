...
func testSetup() (chan *nats.Msg, chan *nats.Msg) {
	...
	nc.ChanSubscribe("rds.create.aws.done", doneChan)
	nc.ChanSubscribe("rds.create.aws.error", errChan)
	...
	Convey("Given I an event", t, func() {
		Convey("With valid fields", func() {
			...
			Convey("When processing the event", func() {
				var e Event
				err := e.Process("rds.create.aws", valid)
			...
			Convey("When validating the event", func() {
				var e Event
				e.Process("rds.create.aws", valid)
				err := e.Validate()
			...
			Convey("When completing the event", func() {
				...
				e.Process("rds.create.aws", valid)
				e.Complete()
				Convey("It should produce ards.create.aws.done event", func() {
			...
			Convey("When erroring the event", func() {
				...
				Convey("It should produce a rds.create.aws.error event", func() {
					msg, timeout := waitMsg(errored)
					So(msg, ShouldNotBeNil)
		...
		Convey("With no datacenter access key", func() {
			...
			Convey("When validating the event", func() {
				var e Event
				e.Process("rds.create.aws", invalid)
				err := e.Validate()
		...
		Convey("With no rds bucket name", func() {
			testEventInvalid := testEvent
			...
			Convey("When validating the event", func() {
				...
				Convey("It should error", func() {
					So(err, ShouldNotBeNil)
					So(err.Error(), ShouldEqual, "rds name is invalid")
				})
			})
		})
